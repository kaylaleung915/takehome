...
*`run_synthetic*.py`, `runs*/`, `verification/V1d*`, `est/controller.py.bak_preA3`, `est/participants_v2.py`, `adherence_check.py`, `adherence.json`, `run_meta.json` and `table.md` cited below are working files from the build that are not included in this repository; the tables and `baselines.json` in this directory carry the results.*
