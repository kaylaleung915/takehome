...
Written 2026-09-08 by a fresh agent after `analyze_trial.py --trial-dir demo_trial_v2` had run. Nothing here is human data. Every number below is traceable to a named file under `demo_trial_v2/` (paths relative to the workspace root). The generator scripts (`make_demo_trial_v2.py`, `make_demo_ceilings_v2.py`, `est/persona.py`), their raw outputs (`sessions/`, `ceiling_cache/`, `pretest_transcripts.jsonl`, `transcripts_with_pretest.jsonl`), `JOURNAL.md` and the derived `results/` files other than `trial_report.md` are not shipped; the pipeline inputs (`participants.csv`, `transcripts.jsonl`, `tasks.yaml`, `ceilings.jsonl`, `truth.json`, `design.json`) and the judge cache (`judge_cache.json`) are, and `analyze_trial.py --trial-dir demo_trial_v2` regenerates `results/`. An independent re-derivation is pending in `verification/V26_demo_trial_v2.md`.
