...
Table not printed for this trial: the control-arm outcome has no variance (mean 0.000, SD 0), so the stratum bounds collapse to a point and quantities scaled by the control SD are undefined. The values are in trial_report.json; the full section prints for a trial whose control arm has a non-degenerate outcome.
